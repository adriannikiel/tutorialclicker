...
public class FlyingObject extends Image {
	...
	private void reactOnClick() {
		...
		if (FlyingObjectType.MONEY.equals(type)) {
			game.getScoreService().addPoints(game.getBalanceService().getMoneyClickValue());
	...
	public void flyLikeHell() {
		...
		Action b = Actions.parallel(Actions.moveBy(xSign * (-500) + (MathUtils.random(-200, 200)), 900, time2),
